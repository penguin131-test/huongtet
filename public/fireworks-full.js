...
// Simple Stage class for canvas management
class Stage {
  ...
  addEventListener(type, listener) {
    ...
    if (type === "pointerstart" || type === "pointerend" || type === "pointermove") {
      const eventMap = { pointerstart: "pointerdown", pointerend: "pointerup", pointermove: "pointermove" }
...
const fscreen = {
  ...
  addEventListener(type, listener) {
    const eventMap = { fullscreenchange: "fullscreenchange" }
    document.addEventListener(eventMap[type] || type, listener)
  },
}

// MyMath utility object
const MyMath = {
  random(min, max) {
...
function getConfigFromDOM() {
  return {
    ...
    scaleFactor: Number.parseFloat(appNodes.scaleFactor.value),
  }
}
...
function setLoadingStatus(status) {
  document.querySelector(".loading-init").textContent = status
}
